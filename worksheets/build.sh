#!/usr/bin/env bash
# Compiles all worksheet.tex files and copies PDFs to docs/worksheet-pdfs/.
# Run from anywhere in the repo: ./worksheets/build.sh

set -uo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"
PDF_OUT="$REPO_ROOT/docs/worksheet-pdfs"
...
for tex_file in "$SCRIPT_DIR"/*/worksheet.tex; do
  ...
  (
    ...
    cp worksheet.pdf "$pdf_name"
    echo "  Saved as $pdf_name in $tex_dir"

    mkdir -p "$PDF_OUT"
    cp "$pdf_name" "$PDF_OUT/$pdf_name"
