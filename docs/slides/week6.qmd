...
# Week 6, Day 1 {.course-title .photo-title data-state="photo-title" background-image="../assets/orange-line-stops-better/stop07-washington-wabash-b.jpg" background-size="cover"}

<h2>Time Series; Communicating Uncertainty</h2>
...
![](../assets/orange-line-map-stops/stop07-washington-wabash.svg){.figure alt="Orange Line map with the Washington/Wabash stop marked, the Week 6 stop"}
...
A **univariate time series** is a sequence of observations of the **same variable** collected over time
- not just a "bag of values"

Two common goals of time series analysis:
...
- Annual spending
- Other time series you can think of?
:::

---

# Example Viz: The Great Decoupling
...
- e.g. "UIC had $X$ graduates last year"
- e.g. "Chicago's budget grew by \$$X$ million last year"
- e.g. "the mortgage interest rate is $X.X$%"
- e.g. "$X$ people have died from the flu this year"
- e.g. "the U.S. spent \$$X$ on social security last year"
:::
...
_(Basically, if peaks get taller as the series continues, use multiplicative.)_
:::
...
- **Summary** displays: error bars, shaded bands, violin plots
- **Frequency** displays: quantile dotplots, icon arrays
- **Ensemble** displays: draw many possible outcomes at once
- **Animated** displays: Hypothetical Outcome Plots (HOPs)


---

# Example: Hurricanes {.section-header}

---

# The Cone of Uncertainty {.image-frame-slide}

The National Hurricane Center's standard forecast graphic uses a point that widens into a "swath" where the storm center might cross.

<img src="../assets/paper-figures/padilla-et-al-cogres-2017-fig1.png" alt="National Hurricane Center track forecast for Hurricane Gustav, 30 August 2008. A black line runs northwest from Cuba into Louisiana with hurricane symbols at forecast positions, surrounded by a white cone that widens with time." style="max-height: 420px; margin-top: 18px;">


---

# What Do People Think the Cone Means?

::: {.incremental}
- The cone shows where the **center** of the storm might go
    - Width comes from *historical forecast error*
    - not from the storm
- But many readers see it as the **size of the storm**, growing over time
- TK: Hullman — a visualization can be accurate and still be misread
:::

---

# Visualizing Multiple Possibilities {.image-frame-slide}

Summary displays (a, c) vs. ensemble displays (b, d) of the same forecast.

<img src="../assets/paper-figures/padilla-et-al-cogres-2017-fig2.png" alt="Four panels over maps of the Gulf Coast. Panels a and c show a shaded blue cone around a single forecast line. Panels b and d show hundreds of individual dark blue tracks fanning out from a common origin." style="max-height: 400px; margin-top: 12px;">

---

# One Hiccup with Multiple POssibilities {.split}

:::: {.split-content}

::: {}
- some readers now over-weight the lines themselves
- "Damage is likely *where a line hits land*, and unlikely between them"
- Each line is one draw, not a literal path/road
:::

<img src="../assets/paper-figures/padilla-et-al-cogres-2017-fig2b.png" alt="Hundreds of individual forecast tracks fanning out from a point near the Texas coast across the Gulf of Mexico, densest in the middle and sparse at the edges.">

::::

---

# Hypothetical Outcome Plots (HOPs) {.split}

:::: {.split-content}

::: {}
Instead of showing the distribution, **animate draws**.

- Each frame is one plausible outcome/draw
- The audience estimates uncertainty by watching mlutiple draws
- Can be more accessible / readable for audiences
:::

<img class="figure" src="../assets/paper-figures/hullman-et-al-plosone-2015-fig2.png" alt="Three depictions of the same distribution: error bars, a violin plot, and selected frames of a Hypothetical Outcome Plot in which a horizontal line jumps to a different value in each frame.">

::::

---

# Live HOP Demo {.image-frame-slide}

Two Orange Line stops, one real weekday per frame. Which stop is busier?

<img src="../assets/hops/hops-ridership-ordering.gif" alt="Animation of a Hypothetical Outcome Plot. Two horizontal marks, labeled Halsted and 35th/Archer, jump to that day's entry count each frame, with the date shown in the corner. Neither stop stays on top: 35th/Archer is higher on a little over half the days. The busier stop each frame is drawn in orange." style="max-height: 440px; margin-top: 10px;">

::: {.notes}
Real CTA weekday entries, sampled at random from 2025 onward.

The flips are not pure noise. Seasonal differences: — 35th/Archer leads on nearly every summer weekday, Halsted through much of the winter. The animation samples random weekdays. Setup for the decomposition material.

Asset regenerated by docs/assets/hops/make_hops_animation.py.
:::
...
::: {.incremental}
- a visualization adopts and/or reinforces a mental model
- ask/explore **"what will people conclude?"**
:::
...
# Week 6, Day 2 {.course-title .photo-title data-state="photo-title photo-title-zoom" background-image="../assets/orange-line-stops-better/stop07-washington-wabash-b.jpg" background-size="cover"}

<h2>Moving Toward Causality</h2>
...
- More ice cream consumption, more drownings
	- Both increase from Summer weather
- The rooster crows, then the sun rises
:::

::: {}
<img class="figure" src="../assets/confounds/fork.svg" alt="Fork DAG: Z at the top with arrows pointing down to X on the left and Y on the right. Z is filled dark to mark the variable to split the data by.">

::: {.caption}
Based on Richard McElreath, *Statistical Rethinking*, 2nd ed. (CRC Press, 2020), ch. 6.
:::
:::
...
::: {}
<img class="figure" src="../assets/confounds/pipe.svg" alt="Pipe DAG: X points to Z, which points to Y, in a horizontal chain. Z is filled dark to mark the in-between step.">
...
::: {}
<img class="figure" src="../assets/confounds/collider.svg" alt="Collider DAG: X on the left and Y on the right both point up to Z. Z is filled orange to warn against splitting the data by it.">
...
# Descendant Example: A Grill {.split}

:::: {.split-content}

::: {}
The gas level $X$ and the igniter $Y$ both feed the flame $Z$, and the flame decides how well the hotdog cooks $D$.

- Imagine nobody wrote down the flame
- The only record is the doneness of the hotdog
- Can the doneness of the hotdog stand in for the flame?
:::

::: {}
<img class="figure" src="../assets/confounds/descendant-collider.svg" alt="Descendant DAG: X on the upper left and Y on the upper right both point down to Z, which points down to D. Z and D are both filled orange to warn against splitting the data by either one.">

::: {.caption}
Example from Christopher Hitchcock, ["Causal Models"](https://plato.stanford.edu/archives/sum2024/entries/causal-models/), *SEP* (2024).
:::
:::
...
- Like a watered-down version of whatever $Z$ was
:::

::: {}
<img class="figure" src="../assets/confounds/descendant.svg" alt="Descendant DAG: X points to Z, which points to Y; Z also points down to D. Z is filled dark and D is filled orange.">
...
7. Gas grill (descendant) example: Christopher Hitchcock, ["Causal Models"](https://plato.stanford.edu/archives/sum2024/entries/causal-models/), *The Stanford Encyclopedia of Philosophy* (Summer 2024) — as discussed in [Chapter 1: Working Toward Wisdom](../ethics-in-data-science/book/01-working-toward-wisdom.html).
8. Uncertainty visualization and the hurricane "cone of uncertainty":
   - Jessica Hullman, ["Confronting Unknowns"](https://doi.org/10.1038/scientificamerican0919-80), *Scientific American* 321(3), p. 80 (September 2019) — published online as "How to Get Better at Embracing Unknowns."
   - Lace M. Padilla, Ian T. Ruginski, Sarah H. Creem-Regehr, ["Effects of Ensemble and Summary Displays on Interpretations of Geospatial Uncertainty Data"](https://doi.org/10.1186/s41235-017-0076-1), *Cognitive Research: Principles and Implications* 2(1):40 (2017) — CC BY 4.0; source of the cone and ensemble figures.
   - Le Liu, Lace Padilla, Sarah H. Creem-Regehr, Donald H. House, ["Visualizing Uncertain Tropical Cyclone Predictions using Representative Samples from Ensembles of Forecast Tracks"](https://doi.org/10.1109/TVCG.2018.2865193), *IEEE TVCG* 25(1):882–891 (2019).
   - Jessica Hullman, Paul Resnick, Eytan Adar, ["Hypothetical Outcome Plots Outperform Error Bars and Violin Plots for Inferences about Reliability of Variable Ordering"](https://doi.org/10.1371/journal.pone.0142444), *PLOS ONE* 10(11):e0142444 (2015) — CC BY 4.0; source of the HOPs figure.
   - Alex Kale, Francis Nguyen, Matthew Kay, Jessica Hullman, ["Hypothetical Outcome Plots Help Untrained Observers Judge Trends in Ambiguous Data"](https://doi.org/10.1109/TVCG.2018.2864909), *IEEE TVCG* 25(1):892–902 (2019).
   - Lace Padilla, Matthew Kay, Jessica Hullman, ["Uncertainty Visualization"](https://doi.org/10.1002/9781118445112.stat08296), *Wiley StatsRef: Statistics Reference Online* (2021).
...
12. Axis label and font sizes: Claus O. Wilke, *Fundamentals of Data Visualization* (O'Reilly, 2019), [ch. 24](https://clauswilke.com/dataviz/small-axis-labels.html). DIKW pyramid figure from [week 1](week1.html).
