...
ax = sns.lineplot(airline, x="Month", y="Passengers")
...
#| echo: false
#| fig-alt: "Line plot of monthly international airline passengers from 1949 to 1960, showing an upward trend with repeating seasonal peaks each summer that grow larger over time"
ax = sns.lineplot(airline.select(["Month", "Passengers"]), x="Month", y="Passengers")
ax.set_ylim(bottom=0)
plt.tight_layout()
```
:::
...
# Spotting the Trend: Rolling Average
...
# Spotting the Trend: Rolling Average {.code-figure-slide}
...
ax = sns.lineplot(airline, x="Month", y="Passengers", alpha=0.35, label="observed")
sns.lineplot(
    airline.filter(pl.col("trend").is_not_null()),
    x="Month", y="trend", label="12-month avg", ax=ax
)
ax.legend()
ax.set_ylim(bottom=0)
plt.tight_layout()
...
Seasonality **scales with the trend** (bigger baseline means bigger swings).
...
# each panel is golden-ratio (5 x 3.09), so the full figure is extra-wide
fig, axes = plt.subplots(1, 2, figsize=(10, 3.09))
for model, ax in zip(["additive", "multiplicative"], axes):
    r = seasonal_decompose(values, model=model, period=12)
...
ax = sns.lineplot(trend_df, x="Month", y="trend")
ax.set_ylim(bottom=0)
...
for df, ax, label in zip([first_two, last_two], axes, ["First 2 years", "Last 2 years"]):
    sns.lineplot(df, x="Month", y="seasonal", ax=ax)
...
ax = sns.lineplot(resid_df, x="Month", y="resid")
ax.axhline(1.0, color="gray", linestyle="--", alpha=0.5)
...
fig, axes = plt.subplots(1, 2, figsize=(10, 3.09))
sns.lineplot(airline, x="Month", y="Passengers", ax=axes[0])
axes[0].set_ylim(bottom=0); axes[0].set_title("Before: Original Series")
sns.lineplot(airline_diff, x="Month", y="first_diff", ax=axes[1])
...
ax = sns.lineplot(uic, x="date", y="rides", linewidth=0.5)
ax.set_ylim(bottom=0)
plt.tight_layout()
```
...
CTA_BLUE = "#00A1DE"
ax = sns.lineplot(uic, x="date", y="rides", linewidth=0.5, color=CTA_BLUE)
ax.set_xlabel("Date")
ax.set_ylim(bottom=0)
plt.tight_layout()
```
...
# Weekly Seasonality {.code-figure-slide}
...
ax = sns.boxplot(uic_dow, x="day_name", y="rides", order=day_order, color=CTA_BLUE)
ax.set_xlabel("")
...
# Decompose UIC-Halsted with Period = 7 {.code-figure-slide}
...
# Weekday-Only Fit: What's Still There? {.code-figure-slide}
...
ax = sns.lineplot(deseasonalized, x="date", y="rides", color=CTA_BLUE, linewidth=0.5)
ax.set_xlabel("Date")
...
# Monthly Trend(s) for UIC-Halsted Stop {.code-figure-slide}
...
ax = sns.boxplot(uic_month, x="month", y="rides", order=month_order, color=CTA_BLUE)
ax.set_xlabel("")
ax.set_ylim(bottom=0)
...
ax = sns.lineplot(wabash, x="date", y="rides", linewidth=0.5, color=CTA_ORANGE)
ax.set_xlabel("Date")
ax.set_ylim(bottom=0)
...
ax = sns.boxplot(wabash_dow, x="day_name", y="rides", order=day_order, color=CTA_ORANGE)
ax.set_xlabel("")
ax.tick_params(axis="x", rotation=30)
...
fig, axes = plt.subplots(1, 2, figsize=(10, 3.09), sharey=True)
sns.boxplot(uic_dow, x="day_name", y="rides", order=day_order, color=CTA_BLUE, ax=axes[0])
sns.boxplot(wabash_dow, x="day_name", y="rides", order=day_order, color=CTA_ORANGE, ax=axes[1])
...
fig, axes = plt.subplots(1, 2, figsize=(10, 3.09), sharey=True)
sns.boxplot(uic_month, x="month", y="rides", order=month_order, color=CTA_BLUE, ax=axes[0])
sns.boxplot(wabash_month, x="month", y="rides", order=month_order, color=CTA_ORANGE, ax=axes[1])
axes[0].set_title("UIC-Halsted (student / commuter)")
axes[1].set_title("Washington/Wabash (downtown Loop)")
for ax in axes: ax.set_xlabel(""); ax.tick_params(axis="x", rotation=45)
plt.tight_layout()
...
- To predict a new day:
    - push **(weekday, month)** through *every* draw
- That gives a **distribution** of outcomes
    — not just a single number!
- The **spread** provides uncertainty
    - only use **mean** if asked 🙂
- e.g. "how many rides on a *Tuesday in December*?" → let's look it up!
:::

---

# Predicting a Specific Day
...
# Predicting a Specific Day {.code-figure-slide}
...
ax = sns.violinplot(draws, x="scenario", y="rides", color=CTA_BLUE, cut=0)
...
# Another Approach: Skip Time Altogether

So far, the date was essentially the model. But ridership responds to factors (**causes??**) that can be identified and measured.
...
- **Special event** like concerts, games, speakers → directly boosts ridership
