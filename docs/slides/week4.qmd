...
- The main tools of exploratory data analysis are:
  - **Summary Statistics** (mean, median, spread, counts, etc.)
  - **Descriptive Visualization** (histograms, box plots, scatter plots)
:::
...
## What happens if you call `.describe()` on a column of strings?
...
EDA is not a fixed recipe. You try a few summaries, make a few plots, and then follow the shape of the data. Some questions are answered by a single number; others require a scatter plot, a histogram, or a comparison across groups.

The point is not to confirm a hypothesis at the start. It is to generate useful questions about the data and to surface assumptions before modeling.
