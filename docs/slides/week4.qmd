...
# Homework 5 {.section-header .no-index}
...
::: {.aside-note}
Week 2 mentioned APIs first, terms of service, [*Sandvig v. Barr*](https://www.aclu.org/sites/default/files/field_document/sandvig_opinion.pdf), etc.
:::

::: {.incremental}
1. **Request** the page: ask the server for the HTML
2. **Parse** that HTML into a structure you can search
3. **Select** the elements that hold the values you want
4. **Extract** the text, one row per observation
5. **Stamp** the row with the time you collected it
6. **Save** a tidy file you can re-read later
:::

---

## Example: CTA Train Tracker at UIC-Halsted {.smaller}

::: {.aside-note}
[CTA Train Tracker: UIC-Halsted arrival estimates](https://www.transitchicago.com/traintracker/arrivaltimes/?sid=40350) (`sid=40350`)
:::

:::: {.columns}

::: {.column width="42%"}
[WEBSITE DISPLAY]{.eyebrow}

> **Service toward O'Hare**
>
> Blue Line #143 to O'Hare &nbsp;&nbsp; **10** min

- The page is live, changes frequently
...
- **class names** (e.g. "ea-line-time-big") will be our selectors
:::
...
- Record `collected` (datetime) **right next to the request**: it stamps when the server answered.
:::
...
`soup.select(...)` takes a **CSS selector** and returns matches in document order: that is what lets one loop handle headings and trains together.
...
## Augmenting Scraped Data {.smaller}

`shown` is what the sign said, maybe not as meaningful as...
...
::: {}
- Remember to look for existing datasets, APIs, etc. before scraping
- **Be polite.** One request a minute or so. Check `/robots.txt`; set a `User-Agent` to identify your bot.
- **The markup will change.** Class names change and scrapers "rot"
- **`Due` is not a number** (at first)
- **Save the raw HTML**, at least while developing. Re-parsing a saved file is free!
- **If the numbers appear only after JavaScript runs**, `requests` will not see them, so you would need a real browser engine, or a way to simulate one 👀
:::
...
## Worked Example: Charli {.smaller}

::: {.aside-note}
The same example from the appendix of [Week 2](week2.html). Lifetime risks from Villeneuve & Mao (1994); attribution figure from the [CDC](https://www.cdc.gov/lung-cancer/risk-factors/index.html). Component names follow *Computational and Inferential Thinking*, [Chapter 18: Updating Predictions](https://inferentialthinking.com/chapters/18/updating-predictions/).
:::
...
::: {.column width="46%"}
- $A$ = Charli has lung cancer &nbsp;&nbsp;
- $B$ = Charli smokes a pack every day
- Terminology:
    - prior &nbsp; $P(A) = 0.04$
...
The prior was 4% (probability that an adult has lung cancer). The evidence roughly **quadrupled** it, to 15%.
:::

::: {.fragment style="font-size:0.85em;"}
Four moving parts: **prior**, **likelihood**, **evidence**, **posterior**.
:::
...
## Bayes' Anatomy: the Probability of A {.split .smaller auto-animate=true}
...
- $P(A)$: the **prior**. What you believe about $A$ *before* any evidence arrives.
    - Charli example: 4% of adults develop lung cancer.
- $P(A \mid B)$: the **posterior**. What you believe about $A$ *after* $B$ is taken into account.
    - Charli example: 15%, once we know Charli smokes a pack a day.
:::

::: {.fragment style="font-size:0.78em; margin-top:0.3em;"}
- Both are statements about **A**.
- The prior is the start, posterior is the end.
:::
...
## Bayes' Anatomy: the Probability of B {.split .smaller}
...
- $P(B \mid A)$: the **likelihood**. How expected $B$ would be *if $A$ were true*.
    - Charli example: 90% of people with lung cancer smoked.
- $P(B)$: the **evidence**, or marginal probability of $B$. How expected $B$ is *overall*, either way.
    - Charli example: 24% of adults smoke.
:::

::: {.fragment style="font-size:0.78em; margin-top:0.3em;"}
$P(B \mid A)$ and $P(A \mid B)$ point in **opposite directions**: swapping them is a common error. $P(B)$ is the normalizer.
:::
...
| What are the columns, types? | `df.glimpse()` | `df.info()` |
...
Note: `shape` is an **attribute**, not a method! `df.shape`, not `df.shape()`.
:::
...
::: {.aside-note}
*Learning Data Science*, [Section 2.2: Target Population, Access Frame, and Sample](https://learningds.org/ch/02/data_scope_construct.html)
:::
...
::: {.aside-note}
*Computational and Inferential Thinking*, [Section 14.2: The Rough Size of Deviations from Average](https://inferentialthinking.com/chapters/14/2/variability/#the-rough-size-of-deviations-from-average)
:::
...
::: {.aside-note}
*Think Stats* (3rd ed.), [Section 4.4: Percentile-Based Statistics](https://allendowney.github.io/ThinkStats/chap04.html#percentile-based-statistics)
:::
...
(ggplot(box, aes("x", "v"))
 ...
 + note(0.3, hi, "Whisker: last point within...?", size=14)
...
:::: {.columns}
::: {.column width="42%"}
::: {style="font-size:0.72em; padding-right:20px;"}
- **Skew** is the direction the long tail points: "the mean chases the tail".
- Dashed line = **mean**
- Solid line = **median**.
...
# Three shapes on one x-axis: a lognormal shifted to sit under the symmetric
# curve, and its mirror image, so the only thing that changes panel to panel is
# which way the tail points.
shapes = pl.DataFrame({
    "shape": np.repeat(SHAPES, 900),
    "v": np.concatenate([1.0 + rng.lognormal(0, 0.75, 900),
                         rng.normal(2, 0.72, 900),
                         3.0 - rng.lognormal(0, 0.75, 900)])}
...
def shape_plot(k):
    """The three panels with only the first k drawn, for the .r-stack reveal.

    The fill and the ink need different opacities when a panel is on, so each
    gets its own identity-scaled alpha column rather than sharing one.
    """
    curves = upto(shapes, "shape", SHAPES, k, on=0.25).with_columns(
        (pl.col("a") > 0).cast(pl.Float64).alias("ink"))
    lines = upto(centers, "shape", SHAPES, k, on=1.0)
    return (ggplot(curves, aes("v"))
     + geom_density(aes(alpha="a"), fill=ORANGE, color="none")
     + geom_line(aes(alpha="ink"), stat="density", color=DARK, size=0.8)
     + geom_vline(lines, aes(xintercept="mean", alpha="a"), color=DARK, size=0.9,
                  linetype="dashed")
     + geom_vline(lines, aes(xintercept="median", alpha="a"), color=TEAL, size=0.9)
     + scale_alpha_identity()
     + coord_cartesian(xlim=(-0.6, 5.2))
     + facet_wrap("shape", ncol=1, scales="free_y")
     + labs(x="", y="")
     + theme_minimal(base_size=13)
     + theme(strip_text=element_text(size=13, weight="bold", color=INK),
             axis_text_y=element_blank(), panel_grid_minor=element_blank(),
             panel_spacing=0.14, figure_size=(5.3, 4.6)))
```
...
#| fig-width: 5.3
#| fig-height: 4.6
#| fig-alt: "Three empty panels labelled right-skewed, symmetric and left-skewed, sharing one x-axis"
shape_plot(0)
```
:::
...
#| fig-width: 5.3
#| fig-height: 4.6
#| fig-alt: "Only the top panel is drawn: a right-skewed density with its long tail to the right, a dashed line at the mean sitting to the right of a solid line at the median"
shape_plot(1)
```
:::
...
#| fig-width: 5.3
#| fig-height: 4.6
#| fig-alt: "The top two panels are drawn: the right-skewed density, and a symmetric density whose mean and median lines sit on top of each other"
shape_plot(2)
```
:::
...
#| fig-width: 5.3
#| fig-height: 4.6
#| fig-alt: "All three panels are drawn: right-skewed, symmetric, and left-skewed, the last with its long tail to the left and the mean line to the left of the median"
shape_plot(3)
```
:::
...
## Chebyshev's Bounds {.text-figure-slide}

::: {.aside-note}
*Computational and Inferential Thinking*, [Section 14.2: Chebychev's Bounds](https://inferentialthinking.com/chapters/14/2/variability/#chebychevs-bounds)
:::

:::: {.columns}
::: {.column width="42%"}
::: {style="font-size:0.72em; padding-right:18px;"}
- A random variable is **$k$ or more SDs from the mean**, at most, $1/k^2$ of the time: $P(|X - \mu| \ge k\sigma) \le 1/k^2$.
- So at least $1 - 1/k^2$ of any distribution sits **within $k$ SDs**.

::: {.fragment fragment-index=3}
- $k = 2$: at least **75%** within, at most **25%** beyond, again, for *any* distribution. No bell curve assumed!
:::
:::
:::
::: {.column width="58%"}
...
# Deliberately nothing like a bell curve: two clumps plus a long right tail.
x = np.concatenate([rng.normal(8, 1.2, 500), rng.normal(16, 1.0, 300),
                    20 + rng.exponential(6, 200)])
mu, sd, K = x.mean(), x.std(ddof=1), 2
lo, hi = mu - K * sd, mu + K * sd
cnt, edges = np.histogram(x, bins=46)
mid = (edges[:-1] + edges[1:]) / 2
WHERE = ["within", "beyond"]
band = pl.DataFrame({"mid": mid, "count": cnt.astype(float),
                     "w": np.repeat(edges[1] - edges[0], len(mid)),
                     "where": np.where(np.abs(mid - mu) < K * sd, WHERE[0], WHERE[1])}
        ).with_columns(pl.col("where").cast(pl.Enum(WHERE)))
top = float(cnt.max())
pad = (x.max() - lo) * 0.06

def cheb_plot(stage):
    """The bound, built up in four passes for the .r-stack reveal.

    Stage 0 is the empty frame, 1 adds the bins, 2 adds the band and the mean
    and SD lines, 3 colors the bins by which side of the band they fall on and
    labels the two shares. Every stage keeps the same scales, so the panel does
    not move as the layers arrive.
    """
    p = ggplot(band, aes("mid", "count"))
    if stage >= 2:
        p += annotate("rect", xmin=lo, xmax=hi, ymin=0, ymax=top * 1.34,
                      fill=ORANGE, alpha=0.08)
    if stage >= 1:
        if stage >= 3:
            p = (p + geom_col(aes(width="w", fill="where"), color="white", size=0.3)
                   + scale_fill_manual({WHERE[0]: ORANGE, WHERE[1]: "#9aa3a6"}))
        else:
            p += geom_col(aes(width="w"), fill="#9aa3a6", color="white", size=0.3)
    if stage >= 2:
        p = (p + geom_vline(xintercept=mu, color=INK, size=0.9)
               + geom_vline(xintercept=[lo, hi], color=DARK, size=0.9, linetype="dashed")
               + note(mu, top * 1.22, "mean", ha="center")
               + note(lo + pad * 0.3, top * 1.31, "mean − 2 SD", ha="left", color=DARK, size=12)
               + note(hi - pad * 0.3, top * 1.31, "mean + 2 SD", ha="right", color=DARK, size=12))
    if stage >= 3:
        p = (p + note(mu, top * 0.80, "at least 75% of the data\nis in here", ha="center", size=14)
               + note(x.max(), top * 0.30, "at most 25%\nout here", ha="right", color="#63696b", size=12))
    return (p
     + scale_y_continuous(limits=(0, top * 1.36), expand=(0, 0))
     + coord_cartesian(xlim=(lo - pad, float(x.max()) + pad))
     + labs(x="", y="count")
     + theme_minimal(base_size=13)
...
#| fig-width: 5.6
#| fig-height: 4.2
#| fig-alt: "An empty plot frame with a count axis and no data drawn yet"
cheb_plot(0)
```
:::
...
#| fig-width: 5.6
#| fig-height: 4.2
#| fig-alt: "A grey histogram of a lumpy two-humped distribution with a long right tail, with no annotations yet"
cheb_plot(1)
```
:::
...
#| fig-width: 5.6
#| fig-height: 4.2
#| fig-alt: "The same grey histogram with a solid line at the mean, dashed lines two standard deviations either side of it, and a lightly shaded band between them"
cheb_plot(2)
```
:::
...
#| fig-width: 5.6
#| fig-height: 4.2
#| fig-alt: "The finished figure: bars inside the band are orange and labelled as at least 75 percent of the data, the bars in the right tail outside it are grey and labelled as at most 25 percent"
cheb_plot(3)
```
:::
...
## The Bound, All the Way Out {.figure-aside .smaller}

::: {.aside-note}
*Computational and Inferential Thinking*, [Section 14.2: Chebychev's Bounds](https://inferentialthinking.com/chapters/14/2/variability/#chebychevs-bounds). Holds for **any** distribution: skewed, bimodal, spiky, anything.
:::

::: {.columns}
::: {.column}
- The bound is **loose** for every particular one
- At $k = 1$ it promises basically nothing: 0% within one SD
- It needs $k > 1$ before it says anything at all
- only reaches 99% at $k = 10$
- Compare $k = 2$: Chebyshev guarantees $1 - 1/2^2 = 75\%$, a normal curve delivers 95%

:::
::: {.column}

::: {.dense-smaller}
| $k$ | Within $k$ SDs of mean: at least $1 - 1/k^2$ | Beyond $k$ SDs from mean: at most $1/k^2$ |
| --- | --- | --- |
| 1 | 0% | 100% |
| $\sqrt{2}$ | 50% | 50% |
| 1.5 | 55.56% | 44.44% |
...
::: {.caption}
Quotation and table from [Chebyshev's inequality](https://en.wikipedia.org/wiki/Chebyshev%27s_inequality), Wikipedia. [CC BY-SA 4.0](https://creativecommons.org/licenses/by-sa/4.0).
:::
...
::: {.aside-note}
*Computational and Inferential Thinking*, [Section 14.3: How to Spot the SD on a Bell-Shaped Curve](https://inferentialthinking.com/chapters/14/3/sd-and-the-normal-curve/#how-to-spot-the-sd-on-a-bell-shaped-curve)
:::
...
- Discussed the habit of *computing* a dataset and never looking at it
- The dashed line is the fitted line on the next four slides
:::
...
::: {style="text-align:center; font-size:0.78em; margin-top:0.3em; line-height:1.9;"}
All four have the same&hellip;
[&nbsp;`n` = 11]{.fragment}[,&nbsp; `mean` of x = 9.00]{.fragment}[,&nbsp; `mean` of y = 7.50]{.fragment}[,&nbsp; `sd` of x = 3.32]{.fragment}[,&nbsp; `sd` of y = 2.03]{.fragment}[,&nbsp; `r` = 0.82]{.fragment}[,&nbsp; fitted line y = 0.5x + 3]{.fragment}[.]{.fragment}

::: {.fragment}
[Every summary number is identical.]{.key}
:::
:::
...
# Appendix: Chebyshev Exercises {.section-header}
...
Three worked exercises on Chebyshev's bound, for anyone who wants the practice:

- Reading the bound off a mean and an SD
- Working backwards from the coverage you want to the interval it costs
- Why the bound is so much looser than the 68-95-99.7 rule
...
::: {.fragment style="font-size:0.9em;"}
$k = 18/6 = 3$. &nbsp; $1/3^2 \approx 0.111$: **at most 11.1%**, and that counts *both* tails together.
:::
...
::: {.fragment style="font-size:0.9em;"}
Which is a *useless* interval: the exam is out of 100. That is the honest lesson, though. A distribution-free guarantee this strong costs almost all your precision.
:::
...
7. Chebyshev's inequality and the empirical rule: *Computational and Inferential Thinking*, [Chapter 14: Why the Mean Matters](https://inferentialthinking.com/chapters/14/why-the-mean-matters), esp. [Section 14.2: Chebychev's Bounds](https://inferentialthinking.com/chapters/14/2/variability/#chebychevs-bounds) and [Section 14.3: How to Spot the SD on a Bell-Shaped Curve](https://inferentialthinking.com/chapters/14/3/sd-and-the-normal-curve/#how-to-spot-the-sd-on-a-bell-shaped-curve). Exercises written for this deck. The full tabulated bound and the quotation on “The Same Bound, All the Way Out” are from [Chebyshev's inequality](https://en.wikipedia.org/wiki/Chebyshev%27s_inequality), Wikipedia, [CC BY-SA 4.0](https://creativecommons.org/licenses/by-sa/4.0).
