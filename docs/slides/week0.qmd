...
::: {.aside-note}
Use `.figure-sm` alongside `.figure` when one image needs to run shorter than
this layout's 430px cap — not a bespoke `max-height`. `.figure-lg` is for
figures that own a whole slide; it does not raise the cap here.
:::

::: {.aside-note}
Add `.golden` (`{.figure-aside .golden}`) when the figure only illustrates a
point the text is making rather. The columns go to φ : 1 — the
:::
