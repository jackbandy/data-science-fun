  ...
  revealjs:
    ...
    include-in-header:
      text: |
        ...
        /* Column widths come from the pipe-table separator row, which pandoc
           turns into a <colgroup>. Those percentages are only honoured under a
           fixed layout -- otherwise the long exception names widen column one. */
        .reveal .narrow-first-col table { table-layout: fixed; width: 100%; }

        /* Wide-format demo: the table is meant to run off the right edge of
           the slide once enough day columns have been added, so nothing here
           may wrap or shrink it back into view. */
        /* Left-aligned, not reveal's centered default: new days have to append
           to the right edge, otherwise the table grows outward both ways. */
        .reveal .wide-demo table {
          border-collapse: collapse; white-space: nowrap;
          margin-left: 0; margin-right: auto; width: auto;
        }
        .reveal .wide-demo th, .reveal .wide-demo td {
          padding: 0.25em 0.7em;
          border-bottom: 1px solid #d8d8d8;
          text-align: right;
        }
        .reveal .wide-demo th:first-child, .reveal .wide-demo td:first-child {
          text-align: left;
          font-weight: 600;
        }
        .reveal .wide-demo thead th { color: #c83214; border-bottom: 2px solid #f9461c; }
        /* Reveal only fades fragments in; a hidden one still holds its column
           open, so the table would start at full width with blank gaps. Taking
           it out of the layout instead is what makes the table visibly grow. */
        .reveal .wide-demo .fragment:not(.visible) { display: none; }
        .reveal .wide-demo td, .reveal .wide-demo th { min-width: 112px; }
        .reveal .wide-demo td:first-child, .reveal .wide-demo th:first-child { min-width: 160px; }

        /* Long format: fixed three columns, so it never leaves the frame. */
        .reveal .long-demo table { border-collapse: collapse; }
        .reveal .long-demo th, .reveal .long-demo td {
          padding: 0.22em 1.4em 0.22em 0; text-align: left;
        }
        .reveal .long-demo thead th { color: #c83214; border-bottom: 2px solid #f9461c; }
        .reveal .long-demo td { border-bottom: 1px solid #d8d8d8; }
        </style>
...
- **Readable** — closer to plain English than other languages, very forgiving
...
5. The numeric labels for OpenAI models (e.g. )
5. TODO add more examples
:::
...
Polars reads the keys as **column names** and each dictionary as a **row**.

---

## The Same Table, Column by Column {.smaller}
A dataframe is a collection of columns, so you can also hand Polars **one list per column**. Same table, built the other way around.
...
```{python}
#| echo: true
students.filter(pl.col("major") == "CS")
```
:::
...
::: {.notes}
- The list-of-dicts version is the shape data arrives in from a web API; the dict-of-lists version is the shape you type by hand
- Point out the dtype row under the header: `str`, `i64`, `str` — Polars inferred them, and it will hold you to them
- `students["grade"]` is a Series; `.mean()` on it gives one number, not a table
:::
...
- **Don't sleeep on Pandas.** Polars/Pandas concepts line up pretty directly, but Pandas is still everywhere.
...
::: {.notes}
- That is the whole thing: import the library, hand `read_csv` a path or a URL, get a DataFrame back
- A URL works exactly like a filename here, so this runs the same on your laptop, in Colab, or on a lab machine
:::
...
`[]` is for quick, interactive pokes at the data. Everything you want to *keep* should go through `select` / `filter` instead.
...
::: {.notes}
- Walk one cell across: Midway on Aug 17 was one cell in the wide table, and is one row here
- Ask what happens tomorrow in each layout — that is the whole argument
- Tall is cheap; wide is expensive. Rows are data, columns are schema
:::
...
::: {.notes}
- The round trip is the point: no information is lost either way, only the layout changes
- Note the date came back as a *string*, not a date — column names are always text, so a pivot loses the dtype
- This is the same pair of verbs as the cheatsheet slide that follows
:::
...
``` markdown
Move cheat sheet to appendix, bring stats up front, make sections more organized... lots more improvements...
```
:::
